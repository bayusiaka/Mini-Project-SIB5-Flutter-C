...
import 'package:techtrack/apis/oppo_detail.dart';

class OppoApi extends StatefulWidget {
  const OppoApi({Key? key}) : super(key: key);

  @override
  State<OppoApi> createState() => _OppoApiState();
}

class _OppoApiState extends State<OppoApi> {
  List<Map<String, dynamic>> oppoData = [];
  ...
  Future<void> fetchOppoApi() async {
    final dio = Dio();
    ...
    try {
      final response = await dio.get('https://mobile-phones2.p.rapidapi.com/82/phones');
  ...
  void navigateToProductDetail(BuildContext context, Map<String, dynamic> product) {
    Navigator.push(
      context,
        MaterialPageRoute(
          builder: (context) => DetailOppo(product: product),
      ),
    );
  }
      ...
      body: Center(
        child: oppoData.isEmpty
            ...
            : GridView.builder(
                ...
                itemCount: oppoData.length,
                itemBuilder: (context, index) {
                  final item = oppoData[index];
